 ...
 *  Dependencies: EdgeWeightedDigraph.java IndexMinPQ.java Stack.java Connection.java
...
//
    ...
    private void relax(Connection e, int airportOrigin, Airplane airplane, String typeOfSearch) {
        ...
        if (typeOfSearch.compareTo("distance") == 0) {
            ...
            if (distTo[w] > distTo[v] + airplane.getFlightDuration(e)) {
                distTo[w] = distTo[v] + airplane.getFlightDuration(e);
                edgeTo[w] = e;
                if (pq.contains(w)) pq.decreaseKey(w, distTo[w]);
                else pq.insert(w, distTo[w]);
            }
    ...
    public Iterable<Connection> pathTo(int v) {
        ...
        for (Connection e = edgeTo[v]; e != null; e = edgeTo[e.from()]) {
            path.push(e);
        }
        return path;
    }
