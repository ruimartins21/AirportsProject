...
public class BreadthFirstPaths {
    ...
    private boolean check(EdgeWeightedDigraph G, int s) {
        ...
        for (int v = 0; v < G.V(); v++) {
            for (Connection e : G.adj(v)) {
                if (hasPathTo(v) != hasPathTo(e.to())) {
                    StdOut.println("edge " + v + "-" + e.to());
                    StdOut.println("hasPathTo(" + v + ") = " + hasPathTo(v));
                    StdOut.println("hasPathTo(" + e.to() + ") = " + hasPathTo(e.to()));
                    return false;
                }
                if (hasPathTo(v) && (distTo[e.to()] > distTo[v] + 1)) {
                    StdOut.println("edge " + v + "-" + e.to());
                    StdOut.println("distTo[" + v + "] = " + distTo[v]);
                    StdOut.println("distTo[" + e.to() + "] = " + distTo[e.to()]);
                    return false;
                }
            }
        }
    ...
    public void printAllConnections(EdgeWeightedDigraph G, int s, BreadthFirstPaths bfs) {
        for (int v = 0; v < G.V(); v++) {
            if (bfs.hasPathTo(v)) {
                StdOut.printf("%d to %d (%d):  ", s, v, bfs.distTo(v));
                for (int x : bfs.pathTo(v)) {
