...
public class SymbolGraph {
    ...
    private AirMap graph;             // the underlying graph
    ...
    public SymbolGraph(String filename, String delimiter) {
        ...
        // line to all others
        graph = new AirMap(st.size());
        in = new In(filename);
        while (in.hasNextLine()) {
            String[] a = in.readLine().split(delimiter);
            int v = st.get(a[0]),ncost = 0;
            for (int i = 1; i < a.length; i+=ncost+1) {
                int w = st.get(a[i]);
                double wind = st.get(a[i+1]);
                float tunnel = st.get(a[i+2]);
                float distance = st.get(a[i+3]);
                Connection c = new  Connection(v,w,wind,tunnel,distance);
                graph.addEdge(c);
            }
        }
    }
